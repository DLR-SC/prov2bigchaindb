...
class DocumentConceptClient(BaseClient):
    ...
    def __init__(self, account_id: str = None, host: str = '0.0.0.0', port: int = 9984, num_connections: int = 1, local_store: local_stores.BaseStore = local_stores.BaseStore()):
        """
...
class GraphConceptClient(BaseClient):
    ...
    def save_document(self, document: str or BufferedReader or provmodel.ProvDocument) -> list:
        ...
        document_tx_ids = []
        prov_document = utils.to_prov_document(content=document)
        elements = utils.get_prov_element_list(prov_document)
        id_mapping = {}
