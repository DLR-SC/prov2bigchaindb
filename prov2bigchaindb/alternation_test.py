...
def main():
    ...
    asset = {'data': {'prov': {}}}
    block = r.table('bigchain').get_all(tx_ids[0]['tx_id'], index='transaction_id')['block']['transactions'].for_each(
        lambda tx: r.branch((tx['id'] == tx_ids[0]['tx_id']), tx.replace({'asset': asset}), tx)).run()
    print(block, end='\n\n')
